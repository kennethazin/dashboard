...
const products = [
  { name: "Cheddar Mild 20kg Block", shortName: "Cheddar Mild" },
  { name: "Butter Unsalted 250g", shortName: "Butter Unsalted" },
  { name: "Whole Milk 2L", shortName: "Whole Milk" },
  { name: "Cheddar Mature 20kg Block", shortName: "Cheddar Mature" },
  { name: "Greek Yogurt Natural 500g", shortName: "Greek Yogurt" },
  { name: "Semi-Skimmed Milk 2L", shortName: "Semi-Milk" },
  { name: "SMP Medium Heat", shortName: "SMP Medium Heat" },
  { name: "WPC80 Instant", shortName: "WPC80 Instant" },
  { name: "Rennet Casein", shortName: "Rennet Casein" },
  { name: "Salted Butter 250g", shortName: "Salted Butter" },
  { name: "Double Cream 500ml", shortName: "Double Cream" },
  { name: "Cheddar Vintage 250g", shortName: "Cheddar Vintage" },
  { name: "Skimmed Milk 1L", shortName: "Skimmed Milk" },
  { name: "Natural Yogurt 1kg", shortName: "Natural Yogurt" },
  { name: "Oat Milk Barista 1L", shortName: "Oat Milk" },
  { name: "Brie de Meaux 200g", shortName: "Brie" },
  { name: "Whipping Cream 250ml", shortName: "Whipping Cream" },
  { name: "Mozzarella Grated 2kg", shortName: "Mozzarella" },
  { name: "Parmesan Wedge 200g", shortName: "Parmesan" },
]

const accounts = ["Tesco", "Dunnes", "Kerry Foods", "Musgrave", "Lidl", "Aldi", "M&S", "SuperValu"]

const planningMonths = ["Jan", "Feb", "Apr", "May", "Jun", "Jul"]

// Generate deterministic-ish values based on account name
const planningItems = accounts.flatMap(account =>
  products.map((prod, pIdx) => {
    // Basic variety in numbers based on account + product index
    const base = (account.length * 100) + (pIdx * 50)
    const values = planningMonths.map((_, mIdx) => {
      const val = base + (mIdx * 120) + (Math.sin(pIdx + mIdx) * 200)
      const absVal = Math.floor(Math.max(100, val))
      return absVal >= 1000 ? `${(absVal / 1000).toFixed(1)}k` : absVal.toString()
    })
    return { account, ...prod, values }
  })
)

const productColors = [
  "#4f855f", "#86bd97", "#284c33", "#d9d6cf", "#6f6b65",
  "#42413c", "#5c7cfa", "#748ffc", "#91a7ff", "#20c997",
  "#38d9a9", "#63e6be", "#fab005", "#fcc419", "#ffd43b",
  "#ff922b", "#ffa94d", "#ffc078", "#e64980", "#f06595"
]
...
function DemandOverviewCard({ selectedProduct }: { selectedProduct: string }) {
  const [duration, setDuration] = useState("1Y")

  // Use selectedProduct to derive unique chart data
  const data = useMemo(() => {
    const item = planningItems.find(p => p.name === selectedProduct) || planningItems[0]

    // Create realistic-looking historical data based on the planning values
      ...
      <div className="mb-4 flex items-start justify-between">
        <div className="space-y-[6px]">
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1.5 rounded-full bg-[#f0f9f3] px-2.5 py-0.5 text-[10px] font-semibold text-[#166534] border border-[#dcfce7] animate-in fade-in slide-in-from-left-1">
...
function PlanningCard({ selectedProduct, onProductSelect }: { selectedProduct: string; onProductSelect: (name: string) => void }) {
    ...
    <article className="overflow-hidden rounded-[12px] border border-[#fafafa] bg-white p-6">
      ...
      <div className="mt-4 flex items-center justify-start gap-2 px-2 py-4">
        ...
        <div className="flex items-center gap-4">
          <p className="text-[14px] text-black">1 - {filteredItems.length} of {filteredItems.length}</p>
